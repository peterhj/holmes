...
fn train_simba_2_layers() {
  ...
  let conv1_layer_cfg = Conv2dLayerConfig{
    in_width: 19, in_height: 19, in_channels: 30,
  ...
  let first_hidden_conv_layer_cfg = Conv2dLayerConfig{
    in_width: 19, in_height: 19, in_channels: 96,
  ...
  let conv2_layer = Conv2dLayer::new(0, first_hidden_conv_layer_cfg, batch_size, Some(&conv1_layer), &ctx);
  ...
  let mut arch = LinearNetArch::new(
      PathBuf::from("experiments/models/tmp_action3_12layer_19x19x30.v2"),
      batch_size,
      data_layer,
  ...
  let dataset_cfg = DatasetConfiguration::open(&PathBuf::from("experiments/gogodb_19x19x30_episode.v2.data"));
