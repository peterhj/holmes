...
pub struct Node {
  ...
  pub state:        TxnState<TxnStateFeaturesData>,
...
impl Node {
  pub fn new(parent_node: Option<&Rc<RefCell<Node>>>, state: TxnState<TxnStateFeaturesData>) -> Node {
    Node{
...
impl Tree {
  pub fn walk(&mut self, tree_policy: &mut TreePolicy, traj: &mut Trajectory) {
    traj.reset();
        ...
        match res {
          Some((place_point, j)) => {
            ...
            if has_child {
              ...
              let leaf_node = Rc::new(RefCell::new(Node::new(Some(&cursor_node), leaf_state)));
...
pub struct McTrajectory {
  pub search_pairs:   Vec<(Rc<RefCell<McNode>>, Point, usize)>,
  pub leaf_node:      Option<Rc<RefCell<McNode>>>,

  state:  TxnState,
  pub rollout_moves:  Vec<(Stone, Point)>,
  pub score:          Option<f32>,
}
...
impl McNode {
  pub fn new<R>(parent: Option<Rc<RefCell<McNode>>>, state: TxnState, rng: &mut R) -> McNode where R: Rng {
...
impl McSearchTree {
  pub fn new_with_root(root_state: FastBoard, root_aux: FastBoardAux) -> McSearchTree {
    ...
    McSearchTree{
      ...
      max_depth:  1000, // FIXME(20151024): for debugging.
